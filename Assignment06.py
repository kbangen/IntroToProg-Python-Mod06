...
# Kevin Bangen, 11/24/2020 ,Modified code to complete assignment 6
# ---------------------------------------------------------------------------- #
...
class Processor:
    ...
    def add_data_to_list(task_input, priority_input, list_of_rows):
        dicRow = {"Task": task_input, "Priority": priority_input.strip()}
        list_of_rows.append(dicRow)
        return list_of_rows, 'Success'

    @staticmethod
    def remove_data_from_list(delete_input, list_of_rows):
        for row in lstTable:
            if (row["Task"] == delete_input):
...
class IO:
    ...
    def input_new_task_and_priority():
        strTask = str(input("Please provide a task:"))
        strPriority = str(input("Please provide a priority:"))
